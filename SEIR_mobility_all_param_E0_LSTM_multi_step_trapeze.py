...
class NeuralNet:
    ...
    def __init__(self, X, Y, I0, hidden_dim, expli, trap, is_pred, batch_size, batch_county, Nm, Nc, Nt, tau, dt, Ntf):
        ...
        self.V, self.c, self.E0, self.gamma, self.delta = self.initialize_LSTM()
        # Define optimizer
        self.optimizer = torch.optim.Adam([self.U_f, self.b_f,self. W_f, 
                                           self.U_i, self.b_i, self.W_i, 
                                           self.U_s, self.b_s, self.W_s, 
                                           self.U_o, self.b_o, self.W_o, 
                                           self.V, self.c, self.E0, self.gamma, self.delta], lr=1e-3, weight_decay=0)
        # Logger
        self.loss_log = []
        self.delta_pred = []
        self.gamma_pred = []
    ...
    def initialize_LSTM(self):      
        ...
        gamma = Variable(self.inv_sigmoid_in(torch.rand(1)).type(self.dtype), requires_grad=True)
        delta = Variable(self.inv_sigmoid_in(torch.rand(1)).type(self.dtype), requires_grad=True)
        
        return U_f, b_f, W_f, U_i, b_i, W_i, U_s, b_s, W_s, U_o, b_o, W_o, V, c, E0, gamma, delta
    ...
    def compute_loss(self, X, Y, I0, idx):
        ...
        p0 = (gamma2-gamma1)*self.sigmoid_in(self.gamma[0,])+gamma1
        p1 = (delta2-delta1)*self.sigmoid_in(self.delta[0,])+delta1
    ...
    def train(self, nIter = 10000):
        ...
        for it in range(nIter):
            ...
            if it % 100 == 0:
                ...
                gamma_pred = (gamma2-gamma1)*self.sigmoid_in(self.gamma)+gamma1
                delta_pred = (delta2-delta1)*self.sigmoid_in(self.delta)+delta1
                self.gamma_pred.append(gamma_pred.cpu().data.numpy())
                self.delta_pred.append(delta_pred.cpu().data.numpy())
                start_time = timeit.default_timer()
...
if __name__ == "__main__":     
    ...
    Nsim = 10 # Number of models to be trained
    Nsave = 0 # The first Nsave models will be discarded and not saved

    gamma1 = 1/8.09 # Lower bound of interval of existence of the parameter \gamma
    gamma2 = 1/3.59 # Upper bound of interval of existence of the parameter \gamma
    delta1 = 1/6.5 # Lower bound of interval of existence of the parameter \delta
    delta2 = 1/1.5 # Upper bound of interval of existence of the parameter \delta
    ...
    for s in range(Nsim):
        ...
        if s>0:
            ...
            model.V, model.c, model.E0, model.gamma, model.delta = model.initialize_LSTM()
            # Re-define optimizer
            model.optimizer = torch.optim.Adam([model.U_f, model.b_f,model. W_f, 
                                                model.U_i, model.b_i, model.W_i, 
                                                model.U_s, model.b_s, model.W_s, 
                                                model.U_o, model.b_o, model.W_o, 
                                                model.V, model.c, model.E0, model.gamma, model.delta], lr=1e-3, weight_decay=0)
            model.loss_log = []
            model.gamma_pred = []
            model.delta_pred = []
        ...
        if s<Nsave:
            ...
            def get_params(): 
                ...
                Weights = torch.cat( ( Weights , model.gamma[:,None] ) , 0 )
                Weights = torch.cat( ( Weights , model.delta[:,None] ) , 0 )
                
                return Variable( Weights , requires_grad=False)
        
            ww = get_params()
            ww = ww.cpu().data.numpy()
            yy = 'all_param_weights/all_param_weights_' + str(s)
            np.save(yy, ww)
            
            # Compute prediction and extrapolation errors
            E0 = 3*I0*sigmoid(model.E0.cpu().data.numpy())
            gamma = (gamma2-gamma1)*sigmoid(model.gamma[0,].cpu().data.numpy())+gamma1
            delta = (delta2-delta1)*sigmoid(model.delta[0,].cpu().data.numpy())+delta1
                        
            loss_list.append( np.log10(np.array(model.loss_log)) )
            gamma_list.append(np.array(model.gamma_pred))
            delta_list.append(np.array(model.delta_pred))
    ...
    yy = 'all_param_plots/all_param_loss.png'
    plt.savefig(yy, dpi = dpiv)
        
    # Save plot of the learned gamma parameter as a function of the iteration number for the trained models
    plt.figure(figsize=(12.5,10))
    for s in range(Nsim-Nsave):  
        col = np.random.rand(3,)
        plt.plot(gamma_list[s], c=col)
    plt.ylabel('$\gamma^*$')
    plt.xlabel('Iteration / 100')
    yy = 'all_param_plots/gamma_traj.png'
    plt.savefig(yy, dpi = dpiv)
     
    # Save plot of the learned delta parameter as a function of the iteration number for the trained models
    plt.figure(figsize=(12.5,10))
    for s in range(Nsim-Nsave):  
        col = np.random.rand(3,)
        plt.plot(delta_list[s], c=col)
    plt.ylabel('$\delta^*$')
    plt.xlabel('Iteration / 100')
    yy = 'all_param_plots/delta_traj.png'
    plt.savefig(yy, dpi = dpiv)

...
if __name__ == "__main__":     
    ...
    def assign_params(Weights):
        ...
        model.E0 = Variable( torch.reshape( Weights[indf+hidden_dim+1:indf+hidden_dim+1+Nc,:], (Nc, 1, 1)) , requires_grad=True)     
        
        model.gamma = Variable( torch.reshape( Weights[indf+hidden_dim+1+Nc:indf+hidden_dim+1+Nc+1,0], (1, )) , requires_grad=True)     
        model.delta = Variable( torch.reshape( Weights[indf+hidden_dim+1+Nc+1:,0], (1, )) , requires_grad=True)     
    ...
    def compute_sol(E0,I0,R0,X,Y,gamma,delta):
        ...
        Rm = Rm.cpu().data.numpy() # Nc x Ntrain x 1
    ...
    for s in range(Nsim):
        ...
        yy = 'all_param_weights/all_param_weights_' + str(s) + '.npy'
        ww = np.load(yy)  
        ...
        CC_list[s,:,:] = compute_sol(E0[ind_err,:,:],I0[ind_err,:,:],R0[ind_err,:,:],X_train[ind_err,:,:],Y_train[ind_err,:],gamma,delta)[:,:,0]  # Nc x Nt x 1
    ...
    yy = 'all_param_plots/sens_avg.png'
    ...
    for i in range(Nc): # Nc
        if Y_train[i,-1]>Ymin:
            ...
            tt = 'all_param_plots/C_R0_' + county_name[i] + "_" + state_name[i] + ".png"
